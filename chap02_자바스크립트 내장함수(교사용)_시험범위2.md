...
- 실습 : (18_number_object.html)
  ...
  ③ toFixed() : 소수점 몇자리까지 보여줄지 결정하는 함수

  ④ toPrecision() : 정수와 소수점 모두를 합쳐사 몇자리까지 보여줄지 결정하는 함수 //실무에서 거의 사용하지 않는다.
  ...
  **\*\*** 자바스크립트의 전역함수 **\*\***
  1. 특정 클래스 내부나 객체내에 정의되지 않고 스크립트 전체 어디에서나 직접 호출이 가능한 함수를 의미한다.
  2. 브라우져 환경에서는 window에 node.js환경에서는 global객체에 소속되어 있다.
  3. 실무에서 많이 사용하는 주요 전역함수
     1)eval() : 문자열로 된 자바스크립트 코드 실행
     2)isFinite() : 값이 유요한 숫자인지 확인
     3)inNaN() : 값이 not a number 인지 확인
...
- 실습 : (19_array_object.html)
  ...
  ② join( ) : 파라미터로 전달된 문다를 이용해서 하나의 문자열로 변환하며 실무에서 많이 사용
  -> join의 활용 : 실무에서 많이 사용하며 많은 데이터양을 동적으로 불러올 경우 매우 효과적이다.
  - 태그 : <body><thead><th><tbody><tr><td>
  - 스타일 : border, margin-top, text-align, background-color
  - 속성 : id(자바스크립트에 document의 위치를 알려주기), class(스타일 대상)

  ③ pop() : 배열의 마지막 요소 변환하고 배열에서 제거

  ④ push() : 배열의 마지막 요소 추가, join과 많이 사용

  ⑤ shift() : 배열의 첫 요소 반환하고 배열에서 제거 => 실무에서 많이 사용 내부적으로 cost가 발생하는 연산자
  ...
  - 정렬
  - 기준 : 문자열(오름차순)
  - 숫자 정렬???? 따라서 숫자일 경우 별도의 알고리즘을 동작시켜야 한다.
    자바스크립트의 비교 함수으 동작원리 =>
    엔진이 내부적으로 알고리즘(크롬의 경우 TimSort알고리즘)을 갖고 있으며
    함수를 매개변수로 사용한다.
    반환값에 따라 다음과 같이 동작한다.
  ...
  실무활용예제 : 사용자가 카테고리를 선택하고, 특정 조건의 상품만

  ⑫ map() : 배열의 모든 요소를 하나씩 돌면서 실행하고 새로운 배열을 생성하는 함수
  실무활용예제 : 서버에서 받아온 json문자열을 화면에 보여줄 html태그나 ui컴퍼넌트 배열로 변환할 때, for~ push() 보다 편의성이 좋다.
